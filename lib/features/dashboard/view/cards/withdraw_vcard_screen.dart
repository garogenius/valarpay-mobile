...
class WithdrawVCardScreen extends ConsumerStatefulWidget {
  final String cardId;
  const WithdrawVCardScreen({super.key, required this.cardId});

  @override
  ConsumerState<WithdrawVCardScreen> createState() => _WithdrawVCardScreenState();
}

class _WithdrawVCardScreenState extends ConsumerState<WithdrawVCardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Withdraw from Card', style: TextStyle(fontSize: 16)),
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text(
                'Move funds from your virtual card back to your USD wallet.',
                style: TextStyle(color: isDark ? Colors.white38 : Colors.grey.shade600, fontSize: 13),
              ),
              const SizedBox(height: 32),
              _buildLabel('Amount to Withdraw', isDark),
              _buildTextField(
                _amountController, 
                '0.00', 
              ...
              FullWidthButton(
                text: _isSubmitting ? 'Processing...' : 'Confirm Withdrawal',
  ...
  Future<void> _handleWithdraw() async {
    ...
    if (mounted) {
      ...
      if (success) {
        context.pop();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Funds withdrawn successfully')));
      } else {
        final error = ref.read(cardNotifierProvider).error ?? 'Failed to withdraw funds';
